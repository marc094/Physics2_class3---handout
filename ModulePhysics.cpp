...
bool PhysBody::Contains(int x, int y) const
{
	...
	for (b2Fixture* fix = body->GetFixtureList(); fix; fix = fix->GetNext()) {
		if (fix->GetShape()->TestPoint(body->GetTransform(), { PIXEL_TO_METERS(x), PIXEL_TO_METERS(y) }))
			ret = true;
	}
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	input.p1 = { PIXEL_TO_METERS(x1), PIXEL_TO_METERS(x2) };
	input.p2 = { PIXEL_TO_METERS(x2), PIXEL_TO_METERS(y2) };
	input.maxFraction = 2.0f;

	for (b2Fixture* fix = body->GetFixtureList(); fix; fix = fix->GetNext()) {
		b2RayCastOutput output;
		if (!fix->RayCast(&output, input, 0)) {
			normal_x = output.normal.x;
			normal_y = output.normal.y;
			ret = (input.p2 - input.p1).Length() * output.fraction;
		}
	}
